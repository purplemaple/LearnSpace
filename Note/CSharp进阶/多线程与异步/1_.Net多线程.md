...
- [线程的终止](https://learn.microsoft.com/en-us/dotnet/standard/threading/pausing-and-resuming-threads)
...
2. 调用`Thread.Interrupt`方法，中断线程的执行  
    ...
    **注：Interrupt 的原理是当子线程空闲时就抛出异常，这时才可被打断，因此如果子线程一直在轮询，根本没有时间来停顿一下时，就打断不了。(如果能让它停止一下，哪怕Sleep(0)，都可以打断)**
    ...
    Console.WriteLine("主线程：结束！");
    ```
1. 使用`CancellationToken`：这是多线程与异步任务通用的方法，此处略，详见[异步任务的取消](3_异步任务的取消.md)

2. 中断线程的老方法：使用`Abrot`，因为会立即强制关闭线程，导致资源泄露等不可预测的问题，现已弃用

#### 对于无法使用`CancellationToken`取消的同步方法（如第三方类库提供，无法修改形参），参考：[10_如何在异步任务中调用及取消一个长耗时的同步方法](10_如何在异步任务中调用及取消一个长耗时的同步方法.md#4.%20开辟单独的进程)
