        ...
        lock (this.waiters)
        {
            ...
            else
            {
                ...
                if (cancellationToken.CanBeCanceled)
                {
                    ...
                    /*
                     *  现在有两条路径可以结束 Task: 1. tcs.TrySetCanceled(ct)， 2. tcs.TrySetException(exception) 
                     *  Cancellation 和 Break 竞态，场景：
                     *  1. Task A 正在等待
                     *  2. token 被 cancel → TrySetCanceled
                     *  3. 同时另一个线程 Break → TrySetException
                     *  谁先执行成功是不确定的, 有的任务收到 TaskCanceledException，有的任务收到 AsyncBarrierBrokenException
                     *  因此，这里决定 Barrier Broken 应该压过 Cancellation
                     */
